...
const CurriculumTable = () => {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-blue-700 mb-6">Curriculum Plans</h1>
              ...
              filteredPlans.map(plan => (
                <tr key={plan.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    <Link to={`/subject/${plan.subject_id}`} className="text-blue-600 hover:underline">
                      {plan.subject}
                    </Link>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <Link to={`/year-group/${plan.year_group_id}`} className="text-blue-600 hover:underline">
                      {plan.year_group}
                    </Link>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <Link to={`/curriculum/${plan.id}`} className="text-blue-600 hover:underline mr-4">
                      View
                    </Link>
                    {isAuthenticated && (user.role === 'admin' || user.role === 'subject_leader') && (
                      <Link to={`/curriculum/edit/${plan.id}`} className="text-green-600 hover:underline">
                        Edit
                      </Link>
                    )}
                  </td>
